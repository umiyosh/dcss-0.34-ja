...
主目的は、DCSS 0.34を日本語で遊べる状態にすることである。副次目標として、AIによる
反復的な品質確認と人間による最終査読を組み合わせ、複数の参加者の指摘を短時間で
翻訳へ反映できる運用を作る。

翻訳は次の短いループで進める。

1. 英語キー、0.34の実装属性、ゲーム内到達性から小さな翻訳単位を確定する。
2. 既存用語と英語原文を参照して下訳する。
3. 原文への忠実性、訳語の一貫性、ファンタジー世界の語感、書式の保持、実表示を
   それぞれ独立した観点として確認し、問題がなくなるまで修正する。
4. AIや自動検査の合格だけで採用済みにせず、人間が最終査読する。
5. PRの行コメントやレビューで受けた指摘は同じPRへ速やかに反映する。大きな一括PRを
   避け、複数人が対象キーと変更理由を追える差分に保つ。
6. 繰り返し現れる用語判断と修正理由は、後続の翻訳で再利用できる規約または用語集へ
   集約する。一度の指摘を一箇所の修正で終わらせず、同種の訳も検索して確認する。
